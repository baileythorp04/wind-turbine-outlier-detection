...
from tools.preprocessing import rename_columns
import pandas as pd
...
def kelmarsh():
    ...
    #   pitch angle
    #   rotor speed
    title = f"{model}_stopped_codes_removed2"


    process_data(n, dataset, model, important_cols, remove_cols=nan_cols, force_remake_files=True, title_modifier=title)
...
def care():
    ...
    title = f"{model}_rotor_under_11_removed"
...
def care_original():
    data = pd.read_csv(f"data/care/3.csv")
    data = rename_columns(data, ["time_stamp", "wind_speed_3_avg", "sensor_50", 'sensor_5_avg', "sensor_52_avg"])
    Three_Curves("Care original", data, filename="3t_original")
